...
public void solicitarRegion(Proceso p, int idReg) {
    if (Simulador.regiones[idReg].enUso()) {
        ...
        Main.sim.mostrar("Se concede la región " + idReg + " al Proceso " + p);
   ...
   void encolar(Proceso pro, int idReg, int modo){
       ...
       int[] cola = new int[2];
        ...
        cola[1]=idReg;
       Main.sim.mostrar("Se encola proceso: "+pro);
...
//        it.}
       ...
       for(int i =0; i<Simulador.colaDeProcesos.size()-1; i++){
           ...
           if (idReg==aux[1]){
                encontrado = true;
                Main.sim.mostrar("Se desencola el proceso " +(char) ('A' +aux[0])+" y se concede la región "+idReg);
...
public void run() {
    if (!this.esCoord()) {
        try {
            ...
            if (this.muriendo) {
                System.out.println("Muriendo antes luego de espera inicial.");
                return;
            }
        }
    }
    Main.sim.mostrar("Iniciando Proceso " + this + " despues de la espera inicial.");
    while (t != null) {
        if (this.enEjecucion) {
            /*
            ...
            if (this.esCoord()) {
                System.out.println("Llamando a controlarProcesos desde run().");
                this.controlarProcesos();
                Thread.yield();
                ...
                if (this.muriendo) {
                    this.terminar();
                    Main.sim.mostrar("El proceso " + this + " está muriendo...");
                    return;
                }
                Main.sim.mostrar(" El proceso " + this + " libera la región " + reg);
    ...
    if (!this.muriendo) {
        Main.sim.mostrar("  Proceso " + this + " ha finalizado.");
    }
    t = null;
}
...
public boolean esCoord(){
    ...
    public void matar(){
        Main.sim.mostrar("El proceso " + this + " termina de manera inesperada.");
        this.muriendo = true;
        t.interrupt();
    }
    ...
    private int buscoRegion() {
        ...
        if (reg == -1){
                ...
                Main.sim.mostrar(" El proceso " + this + " solicita la región " + reg);
        ...
        for(Region r : Simulador.regiones){
            for(Proceso p : Simulador.procesos){
                if (p.muriendo && r.ocupandoRegion(p.idProceso())){
                    Main.sim.mostrar("----------------------------------------------");
                    Main.sim.mostrar("El proceso " + p + " ya no se está ejecutando");
                    Main.sim.mostrar("y mantiene bloqueada la región\n " + r + ".");
                    Main.sim.mostrar("Se fuerza la liberación de dicha región.");
